...
module ethernet_trans(
    ...
    output reg           eth_send_done,         // 新增：以太网发送完成信号（单周期脉冲）
...
parameter  LOCAL_UDP_PORT_NUM = 16'h0002;           // 本地UDP端口号
parameter  LOCAL_IP_ADDRESS   = 32'hc0a8f002;       // 本地IP地址：192.168.240.1
parameter  LOCAL_MAC_ADDRESS  = 48'h0123456789ab;   // 本地MAC地址
parameter  DST_UDP_PORT_NUM   = 16'h0001;           // 目标UDP端口号
parameter  DST_IP_ADDRESS     = 32'hc0a8f001;       // 目标IP地址：192.168.240.2

// ========================= 新增：发送完成检测逻辑 =========================
reg [15:0] send_data_cnt;  // 已发送数据字节计数器
reg        send_in_progress;  // 发送过程标志

// 计数器：跟踪已发送的有效数据字节数
always @(posedge udp_clk or negedge rst_n) begin
    if (!rst_n) begin
        send_data_cnt <= 16'd0;
        send_in_progress <= 1'b0;
    end else begin
        // 开始发送：当应用层请求发送且UDP就绪时，标记发送开始
        if (app_tx_data_request && udp_tx_ready && !send_in_progress) begin
            send_in_progress <= 1'b1;
            send_data_cnt <= 16'd0;
        end
        // 发送中：统计有效数据字节数
        else if (send_in_progress && app_tx_data_valid && udp_tx_ready) begin
            send_data_cnt <= send_data_cnt + 1'b1;
            // 当发送字节数达到总长度时，标记发送完成
            if (send_data_cnt == udp_data_length - 1'b1) begin
                send_in_progress <= 1'b0;  // 发送结束
            end
        end
        // 未发送或发送结束：保持初始状态
        else if (!send_in_progress) begin
            send_data_cnt <= 16'd0;
        end
    end
end
...
always @(posedge udp_clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // 当最后一个字节发送完成时，产生一个周期的脉冲
        eth_send_done <= (send_in_progress && app_tx_data_valid && udp_tx_ready 
                        && (send_data_cnt == udp_data_length - 1'b1)) ? 1'b1 : 1'b0;
    end
end
