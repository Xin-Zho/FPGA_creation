...
(
	...
	*/
    ...
    input                            photo_capture_req,          // 拍照请求（来自拍照模块）
	output reg                       photo_capture_ack,          // 拍照应答（确认收到请求）
	output reg                       photo_done_signal,          // 拍照完成信号（告知拍照模块）
	input[ADDR_BITS - 1:0]           capture_len,                // 拍照数据长度（来自拍照模块）
	input[1:0]                       photo_addr_index            // 拍照地址索引（固定为1，对应write_addr_1）
);
...
reg                                 is_photo_write;             // 1:正在处理拍照写，0:普通写
reg[ADDR_BITS - 1:0]                photo_addr_latch;           // 锁存拍照基地址
...
begin
	...
	begin
        ...
        photo_req_d0 <= photo_capture_req;
		photo_req_d1 <= photo_req_d0;
	end 
end
always @(posedge mem_clk or posedge rst)
begin
	...
	begin
		...
		App_wr_en_d0 <= 1'b0;
        photo_addr_latch <= ZERO[ADDR_BITS - 1:0];
	end
	else begin
		...
		if(state == S_ACK)
		begin
	        if(is_photo_write) begin  // 拍照写：使用拍照专用地址
		        case(photo_addr_index)  // 通常固定为1（write_addr_1）
			        2'd0: App_wr_addr_r <= write_addr_0;
					2'd1: App_wr_addr_r <= write_addr_1;
					2'd2: App_wr_addr_r <= write_addr_2;
					2'd3: App_wr_addr_r <= write_addr_3;
				endcase
                photo_addr_latch <= App_wr_addr_r; 
            end else begin
...
begin
	...
	begin
        ...
        is_photo_write <= 1'b0;
		photo_capture_ack <= 1'b0;
		photo_done_signal <= 1'b0;
		...
		case(state)
			...
			S_IDLE:
			begin
                if(photo_req_d1 == 1'b1 && Sdr_init_done) begin
					state <= S_ACK;
					is_photo_write <= 1'b1;  // 标记为拍照写
					write_len_latch <= capture_len;  // 锁存拍照长度
					photo_capture_ack <= 1'b1;  // 应答拍照请求
				end
			...
			begin
                fifo_aclr <= 1'b1;
                if(is_photo_write) begin
					// 拍照写：无需等待请求撤销，直接进入检查FIFO
					state <= S_CHECK_FIFO;
					fifo_aclr <= 1'b0;  // 清零完成
					photo_capture_ack <= 1'b0;  // 撤销应答
				end
				//after write request revocation(write_req_d2 == '0'),goto 'S_CHECK_FIFO',write_req_ack goto '0'
				    ...
				    begin
					    ...
					    //FIFO reset
					    fifo_aclr <= 1'b1;
					    //select valid base address from write_addr_0 write_addr_1 write_addr_2 write_addr
					    //latch data length
			...
			S_CHECK_FIFO:
			begin
				if(photo_req_d1 == 1'b1) begin
					state <= S_ACK;
					is_photo_write <= 1'b1;
					write_len_latch <= capture_len;
					photo_capture_ack <= 1'b1;
				end
                //if there is a write request at this time, enter the 'S_ACK' state
				else if(write_req_d2 == 1'b1)
				begin
					state <= S_ACK;
				end
			...
			begin
				//burst finish
				if(wr_burst_finish == 1'b1|| (write_cnt + burst_cnt + 1 >= write_len_latch))
				begin
					App_wr_en_r <= 1'b0;
			...
			begin
				state <= S_IDLE;
                if(is_photo_write) begin
					photo_done_signal <= 1'b1;  // 通知拍照模块完成
				end
			end
			default:
				state <= S_IDLE;
